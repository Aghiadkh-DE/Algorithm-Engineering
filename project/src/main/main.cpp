...
int main(int argc, char **argv)
{
    try
    {
        ...
        if (args.threads > 0)
        {
            ...
            if (args.verbose)
                std::cout << "OpenMP threads: " << args.threads << "\n";
        }
        ...
        { // <<< ADD THIS
            ...
            if (args.verbose)
            {
                if (args.suRadius <= 0 || args.suNmin <= 0)
                    std::cout << "Applied SU (auto stroke-width estimation)\n";
                else
        ...
        if (args.morphOpen)
        {
            ...
            if (args.verbose)
                std::cout << "Applied morphology open3x3\n";
        }
        if (args.morphClose)
        {
            result = morphology::close3x3(result);
            if (args.verbose)
                std::cout << "Applied morphology close3x3\n";
        }
        ...
        else if (args.border)
        {
            ...
            if (args.verbose)
                std::cout << "Applied border whiten: w=" << args.borderWidth << "\n";
        }
