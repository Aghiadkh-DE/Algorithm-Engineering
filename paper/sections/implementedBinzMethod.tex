...
The binarization phase is responsible for converting the preprocessed grayscale image into a binary image consisting of only black and white pixels. In our implementation, we provide several methods for binarization, including Otsu's method, Sauvola's method, Nick's method, Su's method, and a proposed method. Most of these methods are implemented as described in the paper, but for some of these methods, such as the proposed method and the Su, Lu, and Tu method, we applied some changes to the original method in order to make it more efficient for some use cases. These modifications, alongside the implementation, will be discussed in the next sections.
...
Nick's method is implemented as a local thresholding algorithm that
...
Sauvola's method is implemented as a local thresholding algorithm in
...
The proposed method is implemented as a local thresholding algorithm
...
The threshold for each pixel is then calculated using the proposed
formula
...
\paragraph{Modification of the Proposed Method}
...
The method proposed by Su, Lu, and Tan (2010) is designed for the
...
boundaries typically produce higher contrast values. \cite{su2010}
